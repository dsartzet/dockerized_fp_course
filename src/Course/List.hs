...
flatMap ::
  ...
  -> List a
  -> List b
-- flatMap fs lst = flatten (map fs lst)
flatMap f =
  flatten . map f
...
notReverse ::
  ...
  -> List a
notReverse =
  error "Epagwgika me ta parapanw props ftanoume oti notReverse(y:.ys) == ... == reverse(y:.ys) to opoio den isxuei ???"
